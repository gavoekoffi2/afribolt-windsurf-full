...
export function ChatInterface({ projectId }: ChatInterfaceProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      type: "agent",
      content:
        "Bonjour! Je suis EMEFA, votre Team Lead AI. Comment puis-je vous aider aujourd'hui?",
      agent: "EMEFA",
      timestamp: new Date(),
    },
  ]);
  ...
  const agents = [
    ...
    { name: "DEDE", role: "Backend", color: "agent-dede" },
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Fallback: provide a helpful response when API is unavailable
      const agentResponse: Message = {
        id: (Date.now() + 1).toString(),
        type: "agent",
        content: `Je suis ${selectedAgent}. Le serveur backend n'est pas connecte pour le moment. Verifiez que le backend est en cours d'execution sur le port 3001.`,
        agent: selectedAgent,
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, agentResponse]);
              ...
              {message.type === "agent" && (
                <div
                  className={`w-8 h-8 rounded-full ${
                    agents.find((a) => a.name === message.agent)?.color ||
                    "bg-gray-400"
      ...
      <div className="border-t border-gray-200 p-4">
        <div className="flex items-center space-x-2">
          <input
            ...
            placeholder={`Message a ${selectedAgent}...`}
