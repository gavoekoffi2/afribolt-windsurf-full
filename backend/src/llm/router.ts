...
export class MultiLLMRouter {
  ...
  async generateResponse(
    ...
    try {
      switch (model) {
        ...
        default:
          if (this.openai) {
            return await this.generateOpenAIResponse("gpt-4", messages, options);
          } else if (this.anthropic) {
            return await this.generateAnthropicResponse("claude-3-sonnet", messages, options);
          } else if (this.gemini) {
            return await this.generateGeminiResponse("gemini-pro", messages, options);
          }
          throw new Error(`Unsupported model: ${model}. No LLM providers configured.`);
      }
    } catch (error) {
      logger.error("LLM generation error:", { model, error: error instanceof Error ? error.message : error });
      throw error;
    }
  }
  ...
  private async generateGeminiResponse(
    ...
    if (!this.gemini) {
      throw new Error("Gemini client not initialized. Set GOOGLE_AI_API_KEY environment variable.");
    }
